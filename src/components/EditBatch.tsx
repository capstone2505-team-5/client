...
interface EditBatchProps {
  onUpdateBatch: (id: string, name: string, rootSpanIds: string[]) => Promise<void>;
}

const EditBatch = ({ onUpdateBatch }: EditBatchProps) => {
  const navigate = useNavigate();
  const { batchId, projectId } = useParams<{ batchId: string; projectId: string }>();
  ...
  // Fetch unique span names for the dropdown - we need to get projectId from batch
  const { data: spanNames = [] } = useUniqueSpanNames(projectId || null);

  // Use edit batch data fetching
  const { data: paginatedData, isLoading } = useEditBatchSpans(
    batchId || null,
  ...
  // Fetch random spans when in random mode (using project spans, not batch spans)
  ...
  useEffect(() => {
    if (batchData && !batchLoading && batchData.batchSummary) {
      setName(batchData.batchSummary.name);
      // Pre-select spans that are already in the batch
      const existingSpanIds = batchData.rootSpans.map(span => span.id);
      setSelectedSet(new Set(existingSpanIds));
    }
  ...
  const rawRootSpans = currentData?.rootSpans || [];
      ...
      <Box sx={{ mb: 2.5, position: 'relative', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', minHeight: 120 }}>
        ...
        <Box sx={{ 
          ...
          {batchData?.batchSummary && (
            <>
              <Tooltip title={<>Back to batch</>} arrow>
                <Box 
                  onClick={() => navigate(`/projects/${projectId}/batches/${batchId}`, { 
                    state: { projectName: projectName, projectId: projectId, batchName: batchData.batchSummary?.name } 
                  })}
                  sx={{
                    px: 2,
                    py: 0.75,
        ...
        >
          ...
          {(isRandomMode || Object.values(appliedFilters).some(value => 
            ...
            <Box sx={{ mt: 1, display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
              ...
              {showSelectedFirst && !isRandomMode && (
                <>
            ...
            slots={{
              noRowsOverlay: () => (
                <Box sx={{ 
                  ...
                  >
                    Try adjusting your filters or check if the project has any data
                  </Typography>
                </Box>
              )
            }}
